...
export const Carousel = ({ items, initialScroll = 0 }: CarouselProps) => {
  ...
  const scrollLeft = () => {
    if (carouselRef.current) {
      carouselRef.current.scrollBy({ left: -392, behavior: "smooth" });
    }
  };
          ...
          >
            {items.map((item, index) => (
              <motion.div
                ...
                key={"card" + index}
                className="last:pr-[5%] md:last:pr-[70%] rounded-3xl"
              >
                {item}
              </motion.div>
            ))}
  ...
  return (
    <>
      ...
      <motion.button
        layoutId={layout ? `card-${card.title}` : undefined}
        // onClick={handleOpen}
        className="rounded-3xl bg-gray-100 dark:bg-neutral-900 h-80 w-56 md:h-[40rem] md:w-[360px] overflow-hidden flex flex-col items-start justify-start relative z-10 border antialiased drop-shadow-xl"
      >
